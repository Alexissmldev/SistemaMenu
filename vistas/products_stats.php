...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 font-sans">
    ...
    <?php
        ...
        $labels_money = []; $data_money = [];
        
        // Clonamos array para ordenar por dinero para la segunda gráfica
        $productos_por_dinero = $productos;
        usort($productos_por_dinero, function($a, $b) { return $b['total_dinero'] <=> $a['total_dinero']; });

        $i = 0;
        foreach($productos as $p){
            if($i < 5) { $labels_qty[] = substr($p['producto_nombre'],0,15); $data_qty[] = $p['cantidad_vendida']; }
            $i++;
        }
        $i = 0;
        foreach($productos_por_dinero as $p){
            if($i < 5) { $labels_money[] = substr($p['producto_nombre'],0,15); $data_money[] = $p['total_dinero']; }
            $i++;
        }
    ?>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left" id="tablaProductos">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <?php endforeach; else: ?>
                    <tr><td colspan="3" class="px-6 py-8 text-center text-gray-500">No hay ventas en este rango de fechas.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
...
<script>
    // 1. Envío de filtros
    function cambiarVista(t){ document.getElementById('tipo_vista').value=t; document.getElementById('filterForm').submit(); }
